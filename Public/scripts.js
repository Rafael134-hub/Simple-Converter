...
// const callback_todas_as_cotacoes = function(erro, res, body){
//     let json = JSON.parse(body)
//     console.log(json)
// }

// request(options, callback_todas_as_cotacoes)
...
function convertCotations() {
    fetch('/api/cotacoes')  
        .then(response => response.json())
        .then(data => {
            let dolar = parseFloat(data.USDBRL.bid);
            console.log('Cotação do dólar:', dolar);

            usdInput.addEventListener("keyup", () => {
                let usdValue = parseFloat(usdInput.value.replace(",", "."));
                if (!isNaN(usdValue)) {
                    brlInput.value = (usdValue * dolar).toFixed(2);
                } else {
                    brlInput.value = 0;
            ...
            brlInput.addEventListener("keyup", () => {
                let brlValue = parseFloat(brlInput.value.replace(",", "."));
                if (!isNaN(brlValue)) {
                    usdInput.value = (brlValue / dolar).toFixed(2);
                }  else {
                    usdInput.value = 0;     
                }
